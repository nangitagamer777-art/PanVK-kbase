                 ...
                 uint32_t handle)
{
   /* Set by default when the device is IO coherent. We might want to
    fprintf(stderr, "[kbase] bo_init: entry, bo=%p dev=%p\n", bo, dev);
   ...
   bo->flags = flags;
   bo->handle = handle;
    fprintf(stderr, "[kbase] bo_init: before refcnt set, handle=%u flags=0x%x\n", bo->handle, bo->flags);
    fprintf(stderr, "[kbase] bo_init: after refcnt set\n");
