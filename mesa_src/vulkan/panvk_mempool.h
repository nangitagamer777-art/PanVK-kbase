...
{
   void *host = panvk_priv_mem_host_addr(mem);

   fprintf(stderr,
           "[kbase_write] START mem=%p host=%p offset=0x%llx\\n",
           (void *)&mem,
           host,
           (unsigned long long)offset);

   void *ptr = (uint8_t *)host + offset;

   fprintf(stderr,
           "[kbase_write] START RETURN ptr=%p\\n",
           ptr);
...
{
   fprintf(stderr,
           "[kbase_write] END ENTER mem=%p offset=0x%llx size=%llu\\n",
           (void *)&mem,
           (unsigned long long)offset,
   ...
   fprintf(stderr,
           "[kbase_write] END bo=%p\\n",
           (void *)bo);

   if (!bo) {
      fprintf(stderr,
              "[kbase_write] END !!! BO NULL !!!\\n");
      return NULL;
   }

   fprintf(stderr,
           "[kbase_write] END kmod_bo=%p flags=0x%x\\n",
           (void *)bo->bo,
           bo->bo ? bo->bo->flags : 0);

   if (bo->bo->flags & PAN_KMOD_BO_FLAG_WB_MMAP) {
      fprintf(stderr,
              "[kbase_write] END WB_MMAP -> FLUSH\\n");

      panvk_priv_mem_flush(mem, offset, size);

      fprintf(stderr,
              "[kbase_write] END FLUSH OK\\n");
   } else {
      fprintf(stderr,
              "[kbase_write] END no flush required\\n");
   }

   fprintf(stderr,
           "[kbase_write] END RETURN\\n");
