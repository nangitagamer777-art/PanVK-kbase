...
#include "csf/panvk_cmd_buffer.h"
...
#if PAN_ARCH >= 10
static void
emit_cs_copy_buffer(struct panvk_cmd_buffer *cmdbuf,
                    uint64_t src_gpu, uint64_t dst_gpu, uint32_t size)
{
   struct cs_builder *b = panvk_get_cs_builder(cmdbuf, PANVK_SUBQUEUE_COMPUTE);

   /* Registros para direcciones */
   struct cs_index src_reg = cs_reg64(b, 0);
   struct cs_index dst_reg = cs_reg64(b, 2);
   struct cs_index data_reg = cs_reg64(b, 4);

   /* Cargar direcciones src y dst (48 bits es suficiente para GPU VA) */
   cs_move48_to(b, src_reg, src_gpu);
   cs_move48_to(b, dst_reg, dst_gpu);

   /* Copiar en chunks de 8 bytes */
   uint32_t offset = 0;
   while (offset + 8 <= size) {
      cs_load64_to(b, data_reg, src_reg, offset);
      cs_store64(b, data_reg, dst_reg, offset);
      offset += 8;
   }

   /* Bytes restantes */
   if (offset + 4 <= size) {
      cs_load32_to(b, cs_reg32(b, 0), src_reg, offset);
      cs_store32(b, cs_reg32(b, 0), dst_reg, offset);
      offset += 4;
   }
   if (offset < size) {
      /* Copiar byte a byte */
      for (uint32_t i = offset; i < size; i++) {
         cs_load32_to(b, cs_reg32(b, 0), src_reg, i);
         cs_store32(b, cs_reg32(b, 0), dst_reg, i);
      }
   }
}
                               ...
                               const VkCopyBufferInfo2 *pCopyBufferInfo)
{
   ...
   /*
    * Keep vkCmdCopyBuffer2() on the real GPU path.
    *
    * CPU memcpy() here would execute during command-buffer recording,
    * not during queue submission, and therefore would not preserve
    * Vulkan command-buffer execution semantics.
    */
   /* Calcular tamano total */
   uint64_t total_size = 0;
   for (uint32_t i = 0; i < pCopyBufferInfo->regionCount; i++)
      total_size += pCopyBufferInfo->pRegions[i].size;

   fprintf(stderr, "[COPY_DEBUG] CmdCopyBuffer2: regionCount=%u total_size=%llu\n",
           pCopyBufferInfo->regionCount,
           (unsigned long long)total_size);

   if (total_size < 16 * 1024 * 1024) {
      /* CPU copy directo durante la grabacion */
      VK_FROM_HANDLE(panvk_buffer, src_buf, pCopyBufferInfo->srcBuffer);
      VK_FROM_HANDLE(panvk_buffer, dst_buf, pCopyBufferInfo->dstBuffer);

      fprintf(stderr, "[COPY_DEBUG] host_ptr: src=%p dst=%p\n", src_buf->host_ptr, dst_buf->host_ptr);
      if (src_buf->host_ptr && dst_buf->host_ptr) {
         for (uint32_t i = 0; i < pCopyBufferInfo->regionCount; i++) {
            const VkBufferCopy2 *region = &pCopyBufferInfo->pRegions[i];
            fprintf(stderr, "[COPY_DEBUG] memcpy: dst=%p + %llu <- src=%p + %llu size=%llu\n",
                    dst_buf->host_ptr, (unsigned long long)region->dstOffset,
                    src_buf->host_ptr, (unsigned long long)region->srcOffset,
                    (unsigned long long)region->size);
            memcpy((uint8_t *)dst_buf->host_ptr + region->dstOffset,
                   (uint8_t *)src_buf->host_ptr + region->srcOffset,
                   region->size);
         }
         fprintf(stderr, "[COPY_DEBUG] CPU copy done\n");

#if PAN_ARCH >= 10
         /* Emit single MOVE32: valid CSF instruction */
         struct cs_builder *b = panvk_get_cs_builder(cmdbuf, PANVK_SUBQUEUE_COMPUTE);
         ...
         fprintf(stderr, "[COPY_DEBUG] CPU copy FAILED: src_host=%p dst_host=%p\n",
                 src_buf->host_ptr, dst_buf->host_ptr);
      }
   }

   /* GPU copy: dispatch directo del shader precompilado */
   struct panvk_precomp_ctx precomp_ctx = panvk_per_arch(precomp_cs)(cmdbuf);
   struct panlib_precomp_grid grid = panlib_1d((total_size + 127) / 128);
   struct panlib_copy_buffer_args args = {0};

   for (uint32_t i = 0; i < pCopyBufferInfo->regionCount; i++) {
      const VkBufferCopy2 *region = &pCopyBufferInfo->pRegions[i];
      VK_FROM_HANDLE(panvk_buffer, src_buf, pCopyBufferInfo->srcBuffer);
      VK_FROM_HANDLE(panvk_buffer, dst_buf, pCopyBufferInfo->dstBuffer);
      args.dst_addr = panvk_buffer_gpu_ptr(dst_buf, region->dstOffset);
      args.src_addr = panvk_buffer_gpu_ptr(src_buf, region->srcOffset);
      fprintf(stderr, "[COPY_GPU] dst_addr=0x%llx src_addr=0x%llx size=%llu\n",
              (unsigned long long)args.dst_addr,
              (unsigned long long)args.src_addr,
              (unsigned long long)region->size);
      grid.count[0] = (region->size + 127) / 128;
      panvk_per_arch(dispatch_precomp)(&precomp_ctx, grid, 0,
                                        PANLIB_COPY_BUFFER, &args, sizeof(args));
   }
}
