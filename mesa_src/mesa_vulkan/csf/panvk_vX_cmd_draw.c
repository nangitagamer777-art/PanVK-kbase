...
alloc_cs_buffer_from_bo(void *cookie)
{
   struct cs_fn_alloc_cookie *c = cookie;
   struct panvk_priv_bo *bo = c->bo;
   size_t remaining = c->total_size - c->offset;
   size_t chunk = remaining < 4096 ? remaining : 4096;
   if (chunk == 0)
      return (struct cs_buffer){0};

   struct cs_buffer buf = {
      .cpu = (uint64_t *)((uint8_t *)bo->addr.host + c->offset),
      .gpu = bo->addr.dev + c->offset,
      .capacity = chunk / sizeof(uint64_t),
   };
   c->offset += chunk;
   return buf;
}
...
{
   /* KBASE diagnostic: test all four CPU writes individually */
   const size_t instr_count = 4;

   fprintf(stderr, "[nop] ENTER cpu=%p capacity=%u\\n",
           (void *)fn_mem.cpu, fn_mem.capacity);

   if (fn_mem.capacity < instr_count) {
      fprintf(stderr, "[nop] ERROR capacity=%u < %zu\\n",
              fn_mem.capacity, instr_count);
      return 0;
   }

   if (!fn_mem.cpu) {
      fprintf(stderr, "[nop] ERROR fn_mem.cpu == NULL\\n");
      return 0;
   }

   uint64_t *cpu = (uint64_t *)fn_mem.cpu;

   fprintf(stderr, "[nop] BEFORE cpu[0] addr=%p\\n", (void *)&cpu[0]);
   cpu[0] = 0x7000000;
   fprintf(stderr, "[nop] AFTER  cpu[0]\\n");

   fprintf(stderr, "[nop] BEFORE cpu[1] addr=%p\\n", (void *)&cpu[1]);
   cpu[1] = 0x7000000;
   fprintf(stderr, "[nop] AFTER  cpu[1]\\n");

   fprintf(stderr, "[nop] BEFORE cpu[2] addr=%p\\n", (void *)&cpu[2]);
   cpu[2] = 0x7000000;
   fprintf(stderr, "[nop] AFTER  cpu[2]\\n");

   fprintf(stderr, "[nop] BEFORE cpu[3] addr=%p\\n", (void *)&cpu[3]);
   cpu[3] = 0x7000000;
   fprintf(stderr, "[nop] AFTER  cpu[3]\\n");

   *dump_region_size = 4096;

   fprintf(stderr, "[nop] LEAVE fn_length=%zu\\n",
           instr_count * sizeof(uint64_t));

   return instr_count * sizeof(uint64_t);
#if 0
   fprintf(stderr, "[kbase_dbg] before panthor_kmod_get_csif_props dev=%p kmod.dev=%p\n",
           dev, dev ? dev->kmod.dev : NULL);

   const struct drm_panthor_csif_info *csif_info =
      panthor_kmod_get_csif_props(dev->kmod.dev);

   fprintf(stderr, "[kbase_dbg] after panthor_kmod_get_csif_props csif_info=%p\n",
           csif_info);

   if (csif_info) {
      fprintf(stderr,
              "[kbase_dbg] csif: csg=%u cs=%u regs=%u scoreboard=%u unpreserved=%u\n",
              csif_info->csg_slot_count,
              csif_info->cs_slot_count,
   ...
   struct cs_builder b;
   fprintf(stderr, "[kbase_dbg] before conf init\n");
   ...
   fprintf(stderr,
           "[kbase_dbg] conf: nr_registers=%u nr_kernel_registers=%u ls_sb_slot=%u fn_mem_size=%zu\n",
           conf.nr_registers,
           conf.nr_kernel_registers,
           conf.ls_sb_slot,
           sizeof(fn_mem));

   fprintf(stderr, "[kbase_dbg] before cs_builder_init b=%p fn_mem_local=%p\n",
           &b, &fn_mem);

   struct cs_fn_alloc_cookie _cookie = {
      .bo = dev->draw_ctx->fns_bo,
      .total_size = fn_mem.capacity * sizeof(uint64_t), /* tamaño del chunk actual */
   };
   conf.alloc_buffer = alloc_cs_buffer_from_bo;
   conf.cookie = &_cookie;

   cs_builder_init(&b, &conf, fn_mem);

   fprintf(stderr, "[kbase_dbg] after cs_builder_init\n");

   struct cs_function function;

   fprintf(stderr, "[kbase_dbg] before cs_subqueue_ctx_reg\n");
   struct cs_index dbg_ctx_reg = cs_subqueue_ctx_reg(&b);
   fprintf(stderr, "[kbase_dbg] after cs_subqueue_ctx_reg\n");
   ...
   fprintf(stderr, "[kbase_dbg] before cs_function_def dump_addr_offset=%zu\n",
           offsetof(struct panvk_cs_subqueue_context, reg_dump_addr));

   cs_function_def(&b, &function, function_ctx) {
      fprintf(stderr, "[kbase_dbg] entered cs_function_def body\n");
      uint32_t fbd_sz = get_fbd_size(has_zs_ext, rt_count);
      fprintf(stderr, "[kbase_dbg] after get_fbd_size, fbd_sz=%u\n", fbd_sz);

      /* argument passed in by the caller */
      fprintf(stderr, "[kbase_dbg] before cs_scratch_reg32(fbd_count)\n");
      struct cs_index fbd_count = cs_scratch_reg32(&b, 0);
      fprintf(stderr, "[kbase_dbg] fbd_count.reg=%u\n", fbd_count.reg);

      /* normal scratch regs */
      fprintf(stderr, "[kbase_dbg] before cs_scratch_reg32(scratch_reg)\n");
      struct cs_index scratch_reg = cs_scratch_reg32(&b, 1);
      fprintf(stderr, "[kbase_dbg] scratch_reg.reg=%u\n", scratch_reg.reg);

      fprintf(stderr, "[kbase_dbg] before cs_scratch_reg64(fbd_addr)\n");
      struct cs_index fbd_addr = cs_scratch_reg64(&b, 2);
      fprintf(stderr, "[kbase_dbg] fbd_addr.reg=%u\n", fbd_addr.reg);

      fprintf(stderr, "[kbase_dbg] before cs_sr_reg64(FRAGMENT, FBD_POINTER)\n");
      struct cs_index fbd_ptr = cs_sr_reg64(&b, FRAGMENT, FBD_POINTER);
      fprintf(stderr, "[kbase_dbg] after cs_sr_reg64, fbd_ptr.reg=%u\n", fbd_ptr.reg);

      cs_add_imm64(&b, fbd_addr, fbd_ptr, 0);
...
get_fn_set_fbds_provoking_vertex_idx(bool has_zs_ext, uint32_t rt_count)
{
   assert(rt_count >= 1 && rt_count <= MAX_RTS);
         fprintf(stderr, "[drawctx] loop: has_zs=%u rt=%u\n", has_zs_ext, rt_count);
...
panvk_per_arch(device_draw_context_init)(struct panvk_device *dev)
{
   ...
   for (uint32_t has_zs_ext = 0; has_zs_ext <= 1; has_zs_ext++) {
      for (uint32_t rt_count = 1; rt_count <= MAX_RTS; rt_count++) {
         ...
         uint32_t _cap = (idx == 0)
            ? (PROVOKING_VERTEX_FN_MAX_SIZE / sizeof(uint64_t))
            : (dev->draw_ctx->fn_set_fbds_provoking_vertex_stride / sizeof(uint64_t));
         fprintf(stderr, "[drawctx] fn_mem: fns_bo=%p host=%p offset=%zu cap=%u\n", (void*)dev->draw_ctx->fns_bo, dev->draw_ctx->fns_bo ? dev->draw_ctx->fns_bo->addr.host : NULL, offset, _cap);
         ...
         fprintf(stderr, "[trace] BEFORE generate_fn\\n");

         size_t fn_length =
            generate_fn_set_fbds_provoking_vertex(dev, fn_mem, has_zs_ext,
                                                  rt_count, &dump_region_size);

         fprintf(stderr, "[trace] AFTER generate_fn fn_length=%zu\\n",
                 fn_length);

         fprintf(stderr, "[trace] dump_region_size addr=%p\\n",
                 (void *)&dump_region_size);

         fprintf(stderr, "[trace] dump_region_size value=%u\\n",
                 dump_region_size);

         fprintf(stderr, "[trace] BEFORE dev dump read\\n");
         fprintf(stderr, "[trace] dev=%p dump_region_size=%p\\n",
                 (void *)dev, (void *)dev->dump_region_size);

         uint32_t old_vertex_dump =
            dev->dump_region_size[PANVK_SUBQUEUE_VERTEX_TILER];

         fprintf(stderr, "[trace] AFTER dev dump read value=%u\\n",
                 old_vertex_dump);

         fprintf(stderr, "[trace] BEFORE regs_test fprintf\\n");

         fprintf(stderr,
                 "[regs_test] provoking: zs=%u rt=%u fn_length=%zu dump=%u vertex_dump_before=%u\\n",
                 has_zs_ext, rt_count, fn_length, dump_region_size,
                 old_vertex_dump);

         fprintf(stderr, "[trace] AFTER regs_test fprintf\\n");

         /* All functions must have the same length */
         fprintf(stderr, "[trace] BEFORE assert\\n");

         assert(idx == 0 ||
                fn_length == dev->draw_ctx->fn_set_fbds_provoking_vertex_stride);

         fprintf(stderr, "[trace] AFTER assert\\n");

         fprintf(stderr, "[trace] BEFORE stride write\\n");

         dev->draw_ctx->fn_set_fbds_provoking_vertex_stride = fn_length;

         fprintf(stderr, "[trace] AFTER stride write\\n");

         fprintf(stderr, "[trace] BEFORE dump MAX2\\n");

         uint32_t new_vertex_dump =
            MAX2(old_vertex_dump, dump_region_size);

         fprintf(stderr, "[trace] AFTER dump MAX2 value=%u\\n",
                 new_vertex_dump);

         fprintf(stderr, "[trace] BEFORE dump write\\n");

         dev->dump_region_size[PANVK_SUBQUEUE_VERTEX_TILER] =
            new_vertex_dump;

         fprintf(stderr, "[trace] AFTER dump write\\n");
      }
   }
