...
init_render_desc_ringbuf(struct panvk_gpu_queue *queue)
{
   ...
   fprintf(stderr, "[kbase] ringbuf->size=%llu, requested=%llu, alignment=%llu\n", (unsigned long long)ringbuf->size, (unsigned long long)(ringbuf->size * 2), (unsigned long long)(ringbuf->size * 2));
   dev_addr = PAN_KMOD_VM_MAP_AUTO_VA; // Usar AUTO_VA igual que los demas buffers

   if (!dev_addr) {
      fprintf(stderr, "[kbase] panvk_as_alloc failed, returning VK_ERROR_OUT_OF_DEVICE_MEMORY\n");
   ...
   ringbuf->addr.dev = vm_ops[0].va.start;
   ...
   fprintf(stderr, "[kbase_dbg] before pool_alloc_mem syncobj\n");
   ringbuf->syncobj = panvk_pool_alloc_mem(&dev->mempools.rw, alloc_info);
   fprintf(stderr, "[kbase_dbg] after pool_alloc_mem, checking alloc\n");
   if (!panvk_priv_mem_check_alloc(ringbuf->syncobj))
      return panvk_errorf(dev, VK_ERROR_OUT_OF_DEVICE_MEMORY,
                          "Failed to create the render desc ringbuf context");

   fprintf(stderr, "[kbase_dbg] before priv_mem_write syncobj\n");

   {
      struct panvk_priv_bo *dbg_bo =
         panvk_priv_mem_bo(ringbuf->syncobj);

      void *dbg_host =
         panvk_priv_mem_host_addr(ringbuf->syncobj);

      uint64_t dbg_dev =
         panvk_priv_mem_dev_addr(ringbuf->syncobj);

      size_t dbg_offset =
         ringbuf->syncobj.offset;

      fprintf(stderr,
              "\n[kbase_syncobj] ================================\n"
              "[kbase_syncobj] mem.bo=%p\n"
              "[kbase_syncobj] priv_bo=%p\n"
              "[kbase_syncobj] kmod_bo=%p\n"
              "[kbase_syncobj] host=%p\n"
              "[kbase_syncobj] dev=0x%llx\n"
              "[kbase_syncobj] offset=0x%zx\n"
              "[kbase_syncobj] =================================\n",
              (void *)ringbuf->syncobj.bo,
              (void *)dbg_bo,
              dbg_bo ? (void *)dbg_bo->bo : NULL,
              dbg_host,
              (unsigned long long)dbg_dev,
              dbg_offset);

      if (dbg_bo) {
         fprintf(stderr,
                 "[kbase_syncobj] handle=%u size=%llu "
                 "bo_flags=0x%x\n",
                 pan_kmod_bo_handle(dbg_bo->bo),
                 (unsigned long long)pan_kmod_bo_size(dbg_bo->bo),
                 dbg_bo->bo->flags);
      }

      fprintf(stderr,
              "[kbase_syncobj] checking /proc/self/maps for %p\n",
              dbg_host);
      ...
      if (maps) {
         char line[512];
         uintptr_t addr = (uintptr_t)dbg_host;
         bool found = false;
         ...
         if (!found) {
            fprintf(stderr,
                    "[kbase_syncobj] !!! NO MAP HIT !!!\n");
         }
      } else {
         fprintf(stderr,
                 "[kbase_syncobj] could not open /proc/self/maps\n");
      }

      fprintf(stderr,
              "[kbase_syncobj] ABOUT TO DIRECT-READ %p\n",
              dbg_host);

      volatile uint32_t *test =
         (volatile uint32_t *)dbg_host;

      uint32_t old_value = *test;

      fprintf(stderr,
              "[kbase_syncobj] DIRECT-READ OK value=0x%x\n",
              old_value);

      fprintf(stderr,
              "[kbase_syncobj] ABOUT TO DIRECT-WRITE %p\n",
              dbg_host);

      *test = 0x534f424a;

      fprintf(stderr,
              "[kbase_syncobj] DIRECT-WRITE OK\n");

      uint32_t new_value = *test;

      fprintf(stderr,
              "[kbase_syncobj] READBACK=0x%x\n",
              new_value);

      *test = old_value;

      fprintf(stderr,
              "[kbase_syncobj] ORIGINAL VALUE RESTORED\n");
   }
   fprintf(stderr, "[kbase_dbg] syncobj host_addr=%p dev_addr=0x%llx\n",
           panvk_priv_mem_host_addr(ringbuf->syncobj),
           (unsigned long long)panvk_priv_mem_dev_addr(ringbuf->syncobj));

   fprintf(stderr, "[kbase_dbg] BEFORE ringbuf syncobj write\n");
   ...
   fprintf(stderr, "[kbase_dbg] AFTER ringbuf syncobj write\n");
   fprintf(stderr, "[kbase_dbg] BEFORE init_render_desc_ringbuf RETURN\\n");
   fprintf(stderr, "[kbase_dbg] ABOUT TO RETURN ZERO, VK_SUCCESS=%d\\n", VK_SUCCESS);
...
init_subqueue(struct panvk_gpu_queue *queue, enum panvk_subqueue_id subqueue)
{
   ...
   fprintf(stderr, "[kbase_dbg] init_subqueue: ENTER subqueue=%u\\n", subqueue);

   fprintf(stderr, "[kbase_dbg] init_subqueue: BEFORE tracing\\n");
   VkResult result = init_subqueue_tracing(queue, subqueue);
   fprintf(stderr, "[kbase_dbg] init_subqueue: AFTER tracing result=%d\\n", result);
   if (result != VK_SUCCESS)
      return result;

   fprintf(stderr, "[kbase_dbg] init_subqueue: BEFORE alloc_info\\n");
   struct panvk_pool_alloc_info alloc_info;
   fprintf(stderr, "[kbase_dbg] init_subqueue: AFTER alloc_info\\n");

   fprintf(stderr,
           "[kbase_dbg] BEFORE dump_region_size: dev=%p subqueue=%u\\n",
           (void *)dev, subqueue);

   fprintf(stderr,
           "[kbase_dbg] sizeof(*dev)=%zu\\n",
           sizeof(*dev));

   fprintf(stderr,
           "[kbase_dbg] dump_region_size FIELD ADDRESS=%p\\n",
           (void *)&dev->dump_region_size);

   fprintf(stderr,
           "[kbase_dbg] AFTER dump_region_size FIELD ADDRESS\\n");
   fprintf(stderr, "[kbase_dbg] init_subqueue: BEFORE dump_region_size access\\n");
   fprintf(stderr, "[kbase_dbg] init_subqueue: dev=%p subq=%p subqueue=%u\\n",
           (void *)dev, (void *)subq, subqueue);
   fprintf(stderr, "[kbase_dbg] init_subqueue: dump_region_size ptr=%p\\n",
           (void *)dev->dump_region_size);
   fprintf(stderr, "[kbase_dbg] init_subqueue: dump_region_size[%u]=%u\\n",
           subqueue, dev->dump_region_size[subqueue]);

   if (dev->dump_region_size[subqueue]) {
      fprintf(stderr, "[kbase_dbg] init_subqueue: ENTER regs_save allocation\\n");
      alloc_info.size = dev->dump_region_size[subqueue];
      alloc_info.alignment = sizeof(uint32_t);
      fprintf(stderr, "[kbase_dbg] init_subqueue: BEFORE regs_save alloc\\n");
      subq->regs_save = panvk_pool_alloc_mem(&dev->mempools.rw, alloc_info);

      fprintf(stderr,
              "[regs_test] regs_save: subqueue=%u size=%u gpu=0x%llx\\n",
              subqueue,
              dev->dump_region_size[subqueue],
              (unsigned long long)panvk_priv_mem_dev_addr(subq->regs_save));
      fprintf(stderr,
              "[kbase_dbg] init_subqueue: AFTER regs_save alloc\\n");
   ...
   fprintf(stderr,
           "[kbase_dbg] init_subqueue: mempool=%p\\n",
           (void *)mempool);
   ...
   fprintf(stderr,
           "[kbase_dbg] init_subqueue: BEFORE req_resource alloc\\n");

   subq->req_resource.buf = panvk_pool_alloc_mem(mempool, alloc_info);

   fprintf(stderr,
           "[kbase_dbg] init_subqueue: AFTER req_resource alloc\\n");
   ...
   fprintf(stderr, "[KBASE] YES CS_BUILDER_INIT\n"); /* KBASE_DIAG_AUTO CS_BUILDER_INIT */
   ...
   fprintf(stderr, "[KBASE] YES BUILDER_FINI\n"); /* KBASE_DIAG_AUTO BUILDER_FINI */
   cs_builder_fini(&b);
   fprintf(stderr, "[KBASE] YES REQ_RESOURCE\n"); /* KBASE_DIAG_AUTO REQ_RESOURCE */
   ...
   fprintf(stderr, "[KBASE] YES CONTEXT_ALLOC\n"); /* KBASE_DIAG_AUTO CONTEXT_ALLOC */
   ...
   fprintf(stderr, "[KBASE] YES CONTEXT_WRITE\n"); /* KBASE_DIAG_AUTO CONTEXT_WRITE */
...
fprintf(stderr, "[KBASE] YES SCOREBOARD\n"); /* KBASE_DIAG_AUTO SCOREBOARD */
#endif
    ...
    * with a seqno of one. */
   fprintf(stderr, "[KBASE] YES SYNCOBJ_WRITE\n"); /* KBASE_DIAG_AUTO SYNCOBJ_WRITE */
   ...
   if (subqueue != PANVK_SUBQUEUE_COMPUTE) {
      ...
      fprintf(stderr, "[KBASE] YES HEAP_SETUP\n"); /* KBASE_DIAG_AUTO HEAP_SETUP */
   ...
   {
      void *_kbase_flush_host = panvk_priv_mem_host_addr(queue->tiler_heap.desc);
      uint64_t _kbase_flush_dev = panvk_priv_mem_dev_addr(queue->tiler_heap.desc);
      size_t _kbase_flush_cs_size = cs_root_chunk_size(&b);
      size_t _kbase_flush_offset = 4096;
      size_t _kbase_flush_end = _kbase_flush_offset + _kbase_flush_cs_size;

      fprintf(stderr, "[KBASE_FLUSH] BEFORE\\n");
      fprintf(stderr, "[KBASE_FLUSH] host=%p\\n", _kbase_flush_host);
      fprintf(stderr, "[KBASE_FLUSH] dev=0x%llx\\n", (unsigned long long)_kbase_flush_dev);
      fprintf(stderr, "[KBASE_FLUSH] offset=%zu\\n", _kbase_flush_offset);
      fprintf(stderr, "[KBASE_FLUSH] cs_size=%zu\\n", _kbase_flush_cs_size);
      fprintf(stderr, "[KBASE_FLUSH] range_end=%zu\\n", _kbase_flush_end);
      fprintf(stderr, "[KBASE_FLUSH] BEFORE panvk_priv_mem_flush\\n");
      /* KBASE_DIAG_AUTO STREAM_FLUSH */
      panvk_priv_mem_flush(queue->tiler_heap.desc, _kbase_flush_offset, _kbase_flush_cs_size);
      fprintf(stderr, "[KBASE_FLUSH] AFTER panvk_priv_mem_flush\\n");
      fprintf(stderr, "[KBASE] YES STREAM_FLUSH\\n");
   }

   fprintf(stderr, "[KBASE_TRACE] AFTER STREAM_FLUSH: before kbase submit\n");

   uint32_t stream_size = cs_root_chunk_size(&b);
   uint64_t stream_addr = cs_root_chunk_gpu_addr(&b);

   fprintf(stderr, "[KBASE_TRACE] kbase submit stream_addr=0x%llx size=%u\n",
           (unsigned long long)stream_addr, stream_size);

   cs_builder_fini(&b);

   fprintf(stderr, "[KBASE] YES MAP_SYNCS\n");
   ...
   if (0 && PANVK_DEBUG(TRACE)) {
      pandecode_user_msg(dev->debug.decode_ctx, "Init subqueue %d binary\n\n",
                         subqueue);
      pandecode_cs_binary(dev->debug.decode_ctx, stream_addr,
                          stream_size,
...
init_queue(struct panvk_gpu_queue *queue)
{
   ...
   fprintf(stderr, "[kbase_dbg] BEFORE init_render_desc_ringbuf CALL\\n");

   result = init_render_desc_ringbuf(queue);

   fprintf(stderr, "[kbase_dbg] AFTER init_render_desc_ringbuf CALL result=%d\\n",
           result);

   if (result != VK_SUCCESS) {
      fprintf(stderr, "[kbase_dbg] init_render_desc_ringbuf FAILED\\n");
      goto err_cleanup_queue;
   }

   fprintf(stderr, "[kbase_dbg] init_render_desc_ringbuf SUCCESS\\n");
...
//       goto err_cleanup_queue;
   ...
   for (uint32_t i = 0; i < PANVK_SUBQUEUE_COUNT; i++) {
      fprintf(stderr, "[kbase_dbg] BEFORE init_subqueue i=%u queue=%p\n", i, (void *)queue);

      result = init_subqueue(queue, i);

      fprintf(stderr, "[kbase_dbg] AFTER init_subqueue i=%u result=%d\\n",
              i, result);
   ...
   if (dev->debug.decode_ctx && PANVK_DEBUG(TRACE))
             ...
             uint32_t shader_core_count)
{
   ...
   /* kbase backend: no group create ioctl; assign dummy group handle */
   queue->group_handle = 0;
   return VK_SUCCESS;
}
...
destroy_group(struct panvk_gpu_queue *queue)
{
   ...
   /* kbase backend: no group destroy ioctl */
   return;
   assert(!ret);
}
...
init_tiler(struct panvk_gpu_queue *queue)
{
    ...
    * kbase backend:
    *
    * Panthor normally creates a real tiler heap through
    * DRM_IOCTL_PANTHOR_TILER_HEAP_CREATE. kbase does not provide
    * that ioctl, so do not issue it here.
    *
    * We still allocate the GPU-visible descriptor and scratch FBD
    * because later PanVK code expects these allocations to exist.
    */
   ...
   tiler_heap->chunk_size =
      phys_dev->csf.tiler.chunk_size;

   alloc_info.size = get_fbd_size(true, MAX_RTS);

#if PAN_ARCH >= 14
   const unsigned fbds_alignment =
      alignof(struct panvk_fb_layer_state);
#else
   const unsigned fbds_alignment =
   ...
   /*
    * No Panthor tiler_heap_create ioctl here.
    *
    * kbase backend uses dummy context values for now.
    */
   tiler_heap->context.handle = 0;
   tiler_heap->context.dev_addr = 0;

   /*
    * Initialize the descriptor so later code sees a valid
    * CPU/GPU-side tiler configuration.
    */
   ...
   /*
    * IMPORTANT:
    * Do NOT jump to err_free_desc here.
    * The allocations belong to the queue and are released by
    * cleanup_tiler().
    */
   return VK_SUCCESS;
}
...
cleanup_tiler(struct panvk_gpu_queue *queue)
{
   ...
   /*
    * kbase backend:
    * there is no Panthor tiler heap object to destroy.
    */
                                ...
                                const struct vk_queue_submit *vk_submit)
{
   ...
   for (uint32_t i = 0; i < vk_submit->command_buffer_count; i++) {
      ...
      for (uint32_t j = 0; j < ARRAY_SIZE(cmdbuf->state.cs); j++) {
         ...
         submit->stream_cpus[submit->qsubmit_count] = b->root_chunk.buffer.cpu ?
         b->root_chunk.buffer.cpu : b->cur_chunk.buffer.cpu;
         submit->qsubmits[submit->qsubmit_count++] =
            (struct drm_panthor_queue_submit){
               .queue_index = j,
...
panvk_queue_submit_ioctl(struct panvk_queue_submit *submit)
{
    ...
    */
   if (dev->kmod.dev) {
      /* kbase backend: submit cada stream individualmente */
      for (uint32_t i = 0; i < submit->qsubmit_count; i++) {
         const struct drm_panthor_queue_submit *qsubmit = &submit->qsubmits[i];
         uint64_t stream_gpu = qsubmit->stream_addr;
         uint32_t stream_size = qsubmit->stream_size;
         
         /* CPU pointer: el stream vive en el cs_pool del cmdbuf.
          * Como no tenemos el cmdbuf aqui, usamos un fallback seguro. */
         void *stream_cpu = submit->stream_cpus[i];

         if (!stream_cpu) {
            uint64_t tiler_gpu = panvk_priv_mem_dev_addr(queue->tiler_heap.desc);
            void *tiler_host = panvk_priv_mem_host_addr(queue->tiler_heap.desc);
            if (tiler_host && stream_gpu >= tiler_gpu) {
               stream_cpu = (void *)((uint8_t *)tiler_host + (stream_gpu - tiler_gpu));
         ...
         /* Saltar streams vacios */
         if (stream_size == 0 || stream_gpu == 0) {
            fprintf(stderr, "[KBASE_SUBMIT_IOCTL] qsubmit[%d]: SKIP vacio (size=%u)\n",
                    i, stream_size);
            continue;
         }

         fprintf(stderr, "[KBASE_SUBMIT_IOCTL] qsubmit[%d]: stream_gpu=0x%llx size=%u\n",
                 i, (unsigned long long)stream_gpu, stream_size);
         
         int ret = kbase_cs_queue_submit(dev->kmod.dev, stream_gpu, stream_cpu, stream_size);
         if (ret < 0) {
            fprintf(stderr, "[KBASE_SUBMIT_IOCTL] submit failed: %d\n", ret);
            return vk_queue_set_lost(&queue->vk, "kbase submit failed");
         }

         /* Flush de coherencia CPU/GPU despues del submit.
          * En Mali unificada, la GPU escribe en la RAM compartida.
          * La CPU necesita invalidar su cache para ver los datos. */
         if (stream_cpu && stream_size > 0) {
            __builtin___clear_cache((char *)stream_cpu,
                                    (char *)stream_cpu + stream_size);
         }
      }
      return VK_SUCCESS;
   }
                                 ...
                                 const struct vk_queue_submit *vk_submit)
{
   ...
   if (decode_ctx && PANVK_DEBUG(TRACE))
...
{
   fprintf(stderr, "[kbase] entered create_gpu_queue\n");
   ...
   fprintf(stderr, "[kbase] before drmSyncobjCreate, drm_fd=%d\n", dev->drm_fd);
   queue->syncobj_handle = 0; /* kbase no usa syncobj DRM */
   int ret = 0;
   if (ret) {
   ...
   fprintf(stderr, "[kbase] before init_tiler\n");
   result = init_tiler(queue);
   fprintf(stderr, "[kbase] after init_tiler, result=%d\n", result);
   if (result != VK_SUCCESS)
   ...
   fprintf(stderr, "[kbase] before create_group\n");
   result = create_group(queue, get_panthor_group_priority(create_info),
                         core_ctrl ? core_ctrl->shaderCoreCount : 0);
   fprintf(stderr, "[kbase] after create_group, result=%d\n", result);
   if (result != VK_SUCCESS)
      goto err_cleanup_tiler;

   fprintf(stderr, "[kbase] before init_queue\n");
   result = init_queue(queue);
   fprintf(stderr, "[kbase] after init_queue, result=%d\n", result);
   if (result != VK_SUCCESS)
...
panvk_per_arch(destroy_gpu_queue)(struct vk_queue *vk_queue)
{
   ...
   if (dev->kmod.dev) {
      /* kbase backend: solo kbase_cs_queue_destroy (ya hace cleanup del ring) */
   ...
   vk_queue_finish(&queue->vk);
   /* NO vk_free: el framework de Mesa (vk_foreach_queue_safe) lo libera */
}
...
panvk_per_arch(gpu_queue_check_status)(struct vk_queue *vk_queue)
{
   ...
   /* kbase: poll output_page para verificar idle */
   uint32_t *output_page = pan_kmod_get_output_page();
   if (output_page) {
      for (int i = 0; i < 1000; i++) {
         if (output_page[4] == 0)  /* CS_ACTIVE = 0 → idle */
            return VK_SUCCESS;
         usleep(1000);
      }
   }
