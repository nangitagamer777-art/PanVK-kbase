...
finish_cs(struct panvk_cmd_buffer *cmdbuf, uint32_t subqueue)
{
   ...
   /* kbase: skip if builder has no stream */
   if (b->cur_chunk.buffer.capacity == 0)
      return;
...
{
   /* kbase: no hay timestamp queries, no generar CS */
      return;
...
panvk_per_arch(EndCommandBuffer)(VkCommandBuffer commandBuffer)
{
   ...
   for (uint32_t i = 0; i < ARRAY_SIZE(cmdbuf->state.cs); i++) {
      ...
      /* Skip builders no inicializados */
      if (b->cur_chunk.buffer.capacity == 0)
         continue;
...
alloc_cs_buffer(void *cookie)
{
   ...
   const unsigned capacity = 4 * 1024 * 1024 / sizeof(uint64_t);

   struct pan_ptr ptr = panvk_cmd_alloc_dev_mem(cmdbuf, cs, capacity * 8, 64);
   fprintf(stderr, "[cs_alloc] cpu=%p gpu=0x%llx size=%zu\n", ptr.cpu, (unsigned long long)ptr.gpu, (size_t)capacity * 8);
...
init_cs_builders(struct panvk_cmd_buffer *cmdbuf)
{
   ...
   if (!csif_info || csif_info->cs_reg_count == 0) {
      struct kbase_csif_info *kbase_csif = kbase_get_csif();
      csif_info = (const struct drm_panthor_csif_info *)kbase_csif;
   }
