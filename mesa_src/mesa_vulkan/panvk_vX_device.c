                              ...
                              VkDevice *pDevice)
{
   ...
   /* Populate cmd_dispatch for PAN_ARCH > 9 too */
   vk_device_dispatch_table_from_entrypoints(
      &device->cmd_dispatch, &panvk_per_arch(device_entrypoints), true);
   vk_device_dispatch_table_from_entrypoints(&device->cmd_dispatch,
   ...
   vk_device_dispatch_table_from_entrypoints(&dispatch_table,
	...
	fprintf(stderr, "[kbase] vk_device_init returned %d\n", result);
   ...
   /* Inyectar manualmente entrypoints faltantes para PAN_ARCH > 9 */
   device->vk.dispatch_table.CmdCopyBuffer2 = panvk_per_arch(CmdCopyBuffer2);
   device->vk.dispatch_table.BeginCommandBuffer = panvk_per_arch(BeginCommandBuffer);
   device->vk.dispatch_table.EndCommandBuffer = panvk_per_arch(EndCommandBuffer);
   fprintf(stderr, "[kbase] Entrypoints injected: Copy=%p Copy2=%p Begin=%p End=%p\n",
           (void *)device->vk.dispatch_table.CmdCopyBuffer,
           (void *)device->vk.dispatch_table.CmdCopyBuffer2,
           (void *)device->vk.dispatch_table.BeginCommandBuffer,
           (void *)device->vk.dispatch_table.EndCommandBuffer);
   ...
   device->kmod.dev = pan_kmod_dev_create(
	...
	fprintf(stderr, "[kbase] device->kmod.dev check passed\n");
   if (!device->kmod.dev) {
   ...
   device->kmod.vm =
	...
	fprintf(stderr, "[kbase] pan_kmod_vm_create: %p\n", device->kmod.vm);
	fprintf(stderr, "[kbase] after vm_create, continuing...\n");
   if (!device->kmod.vm) {
...
#if PAN_ARCH >= 10
   const int *csif_info = (const int *)
      panthor_kmod_get_csif_props(device->kmod.dev);

   if (csif_info[0] > 16) fprintf(stderr, "[kbase] WARN: scoreboard_slot_count=%d\n", csif_info[0]);
   device->csf.sb.count = csif_info[0];
   device->csf.sb.all_mask = (uint16_t)BITFIELD_MASK(csif_info[0]);

   if (device->csf.sb.count <= PANVK_SB_ITER_START) fprintf(stderr, "[kbase] WARN: sb.count=%d <= %d\n", device->csf.sb.count, PANVK_SB_ITER_START);
...
#if PAN_ARCH <= 9
    fprintf(stderr, "[kbase] about to create tiler_heap\n");
   ...
   if (result != VK_SUCCESS)
      goto err_free_priv_bos;
    fprintf(stderr, "[kbase] about to create sample_positions\n");
   ...
   if (result != VK_SUCCESS)
      goto err_free_priv_bos;
#endif
    fprintf(stderr, "[kbase] about to create sample_positions (post JM block)\n");
   ...
   if (result != VK_SUCCESS)
    ...
    /* kbase skip */    pan_upload_sample_positions(device->sample_positions->addr.host);
    /* kbase skip */    panvk_priv_bo_flush(device->sample_positions, 0,
                       pan_sample_positions_buffer_size());

    #if PAN_ARCH >= 10
   result = VK_SUCCESS; /* kbase: skip init_tiler_oom */
   if (result != VK_SUCCESS)
      goto err_free_priv_bos;
#endif
    fprintf(stderr, "[kbase] about to create printf buffer\n");
   ...
   vk_device_set_drm_fd(&device->vk, device->kmod.dev->fd);
    fprintf(stderr, "[kbase] after drm_fd setup\n");


   result = panvk_precomp_init(device);
    fprintf(stderr, "[kbase] after precomp_init, result=%d\n", result);
   if (result != VK_SUCCESS)
      goto err_free_priv_bos;
   ...
   device->vk.mem_cache = vk_pipeline_cache_create(&device->vk, &cache_info, NULL);
    fprintf(stderr, "[kbase] after pipeline_cache_create\n");
   ...
   result = panvk_per_arch(device_draw_context_init)(device);
    fprintf(stderr, "[kbase] after device_draw_context_init, result=%d\n", result);
   if (result != VK_SUCCESS)
      goto err_free_mem_cache;
#endif

   result = panvk_meta_init(device);
    fprintf(stderr, "[kbase] after meta_init, result=%d\n", result);
   if (result != VK_SUCCESS)
