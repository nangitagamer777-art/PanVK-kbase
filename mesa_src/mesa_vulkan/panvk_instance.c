                     ...
                     VkInstance *pInstance)
{
   ...
   *pInstance = panvk_instance_to_handle(instance);
   /* If no DRM devices found, try /dev/mali0 */
	fprintf(stderr, "[kbase] trying fallback to /dev/mali0...\\n");
   if (list_is_empty(&instance->vk.physical_devices.list)) {
      struct vk_physical_device *pdev = NULL;
      panvk_physical_device_try_create(&instance->vk, NULL, &pdev);
      if (pdev)
