...
const struct pan_model pan_model_list[] = {
   ...
   VALHALL_MODEL(PAN_PROD_ID(10, 8, 7), 0, "G610",   "G710", MODEL_ANISO(ALL),  MODEL_TB_SIZES(32768, 16384),
	...
	VALHALL_MODEL(PAN_PROD_ID(10, 8, 7), 1, "G615",   "G710", MODEL_ANISO(ALL),  MODEL_TB_SIZES(32768, 16384), 0, 0),
   ...
   VALHALL_MODEL(PAN_PROD_ID(10, 12, 4), 4, "G310v5",   "G710", MODEL_ANISO(ALL),  MODEL_TB_SIZES(32768, 16384),
                                              MODEL_RATES_X(4, 8, 16,  64,  64, 16)),
	VALHALL_MODEL(PAN_PROD_ID(10, 8, 7), 1, "G615",   "G710", MODEL_ANISO(ALL),  MODEL_TB_SIZES(32768, 16384), 0, 0),
	VALHALL_MODEL(PAN_PROD_ID(10, 8, 7), 0, "G615",   "G710", MODEL_ANISO(ALL),  MODEL_TB_SIZES(32768, 16384), 0, 0),
...
pan_get_model(uint64_t gpu_id, uint32_t gpu_variant)
{
   ...
   for (unsigned i = 0; i < ARRAY_SIZE(pan_model_list); ++i) {
	...
	if (pan_model_list[i].gpu_prod_id == gpu_prod_id) { fprintf(stderr, "[kbase] fallback found model %s at i=%u\n", pan_model_list[i].name, i); return &pan_model_list[i]; }
   }

   /* Fallback: ignore gpu_variant */ for (unsigned i = 0; i < ARRAY_SIZE(pan_model_list); ++i) { if (pan_model_list[i].gpu_prod_id == gpu_prod_id) return &pan_model_list[i]; } /* Fallback failed */ fprintf(stderr, "[kbase] fallback FAILED gpu_prod_id=0x%x\n", gpu_prod_id); return NULL;
}
