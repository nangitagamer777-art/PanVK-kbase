...
{
   const char *path; if (drm_device) path = drm_device->nodes[DRM_NODE_RENDER]; else path = "/dev/mali0";
   ...
   if (!version) {
      fprintf(stderr, "[kbase] drmGetVersion failed on %s, trying kbase backend anyway\n", path);
   } else {
      if (strcmp(version->name, "panfrost") && strcmp(version->name, "panthor") && strcmp(version->name, "kbase")) {
                           ...
                           drmDevicePtr drm_device)
{
   ...
   if (drm_device)
      ...
      result = VK_SUCCESS;
   fprintf(stderr, "[kbase] after get_drm_device_ids: result=%d\n", result);
   if (result != VK_SUCCESS)
      goto fail;
